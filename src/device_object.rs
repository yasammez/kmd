...
extern "fastcall"
{
    ...
    pub fn IoDetachDevice(DeviceObject: *mut DEVICE_OBJECT);
    ...
    pub fn IoAttachDeviceToDeviceStackSafe(SourceDevice: *mut DEVICE_OBJECT, TargetDevice: *mut DEVICE_OBJECT, AttachedToDeviceObject: *const *mut DEVICE_OBJECT) -> NTSTATUS;
    pub fn IoCallDriver(DeviceObject: *mut DEVICE_OBJECT, irp: &mut IRP) -> NTSTATUS;
}
...
pub struct DEVICE_OBJECT
{
	...
	pub NextDevice: Option<NonNull<DEVICE_OBJECT>>,
...
pub type PDRIVER_CANCEL = Option<extern "fastcall" fn (DeviceObject: &mut DEVICE_OBJECT, Irp: &mut IRP)>;
pub type PDRIVER_DISPATCH = Option<extern "fastcall" fn (DeviceObject: &mut DEVICE_OBJECT, Irp: &mut IRP) -> NTSTATUS>;
pub type PIO_COMPLETION_ROUTINE = Option<extern "fastcall" fn (DeviceObject: &mut DEVICE_OBJECT, Irp: &mut IRP, Context: PVOID) -> IO_COMPLETION_ROUTINE_RESULT>;

/// Iterate over devices
pub struct Devices {
    curr: Option<NonNull<DEVICE_OBJECT>>
}

impl Iterator for Devices {
    type Item = *mut DEVICE_OBJECT;
    fn next(&mut self) -> Option<Self::Item> {
        match self.curr {
            Some(device) => {
                let tmp = device.as_ptr();
                unsafe { self.curr = (*tmp).NextDevice; }
                Some(tmp)
            },
            None => None
        }
...
impl IntoIterator for &DRIVER_OBJECT {
    type Item = *mut DEVICE_OBJECT;
    type IntoIter = Devices;
    fn into_iter(self) -> Self::IntoIter {
        Devices {
            curr: self.DeviceObject
        }
    }
}
